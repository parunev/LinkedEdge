...
@Operation(summary = "Get all user education's", description = "GET endpoint to retrieve all education entries associated with the user's profile. The user needs to be logged in order to do this.")
@ApiResponse(
        responseCode = "200",
...
@ApiResponse(
        ...
        description = "This response is for the case where the user or the profile is not found in the database or the Security Context Holder.\n" +
                "Another possible response a user can get here is 'You currently have no education records.'",
        content = {@Content(mediaType="application/json", schema = @Schema(implementation = ApiError.class))}
)
public @interface ApiAllEducations {
}
